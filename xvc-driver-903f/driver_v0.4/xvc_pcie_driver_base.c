		...
		for (d = 0; d < CHAR_DEVICES_MAX; ++d) {
			if (xil_xvc_devices[d * USER_CONFIG_COUNT].pci_dev == NULL) {
				for (u = 0; u < USER_CONFIG_COUNT; ++u) {
					...
					if (name && name[0]) {
						sprintf(ioc_device_name, "xil_xvc/903f_%d_%s", d, name);
					} else if (index_used) {
						sprintf(ioc_device_name, "xil_xvc/903f_%d-%d", d, index_used);
						++index_used;
					} else {
						sprintf(ioc_device_name, "xil_xvc/903f_%d", d);
						++index_used;
					}
...
static int __init xil_xvc_init(void)
{
	...
	xvc_dev_class = class_create(THIS_MODULE, "xil_xvc_903f_class");
	if (IS_ERR(xvc_dev_class)) {
		xil_xvc_cleanup();
